<?
	...
	require_once("../../lib/modelo/sqls/viaticos/Viarlisrub.class.php");
	...
	class pdfreporte extends fpdf
	{
		...
		function header()
		{
			...
			$this->setwidths(array(20,80,50,40));
			$this->setAligns('C');
                        $this->setJump(5);
                        $this->setFillColor(155,155,155);
                        $this->setBorder(true);
                        $this->setFilltable(1);
			$this->Rowm(array("CODIGO RUBRO","DESCRIPCION RUBRO","TIPO DE CALCULO","PARTIDA"));
                        $this->setFilltable(0);
		...
		{                    
		    ...
		    foreach($this->arrp as $arr)
		    {
		    	$this->Rowm($arr);
                    ...
                    $this->setFont("arial","B",11);
                    $this->setTextColor(0,0,155);
                    $this->multicell(180,5,'TOTAL CANTIDAD DE RUBROS:  '.$i);
		}
	}