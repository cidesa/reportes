...
$dinamico=$opciones["Parametros"]["dinamico"];
$jasper=$opciones["Parametros"]["jasper"];
$generatxt=$opciones["Parametros"]["generatxt"];
if($dinamico=='S')
{    
    $_SESSION['dinamic'] = $dinamico;
}
 ?>
...
foreach($opciones["Filas"] as $opc)
{
	$label=$opc["label"];//"PRUEBA ETIQUETA";
        if($jasper=='S')
            $sql=str_replace("@",",",$opc["sql"]);//"SELECT min(codnom) as codnommin,max(codnom) as codnommax FROM npnomina";
        else
