 ...
 ";
	...
	function Cuerpo()
	{
		...
		while(!$this->tbg->EOF)
		{
			...
			$cuantos = $this->bd->select("SELECT COUNT(DISTINCT(CODEMP)) as numero FROM NPNOMCAL WHERE trim(asided)=trim('".$this->tbg->fields["asided"]."') 
                      and trim(codcon) = trim('".$this->tbg->fields["codcon"]."')")->fields["numero"];
			if(strtoupper($this->tbg->fields["asided"])=="A")
			{
