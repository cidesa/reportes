<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->benfac=$_POST["benfac"];
				...
				$this->sql="select a.numord,to_char(b.fecemi,'dd/mm/yyyy') as fecemi,b.cedrif,b.nomben,c.nitben, c.dirben, c.telben,to_char(a.fecfac,'dd/mm/yyyy') as fecfac,to_char(b.fecpag,'dd/mm/yyyy') as fecpag,a.numfac,
							a.numctr,a.tiptra,a.facafe,
							a.totfac,a.exeiva,a.basislr,a.porislr,a.monislr , b.monord as monto
							from opfactur a,  opordpag b join opbenefi c on b.cedrif=c.cedrif 
							,opbenefi e
                            where a.numord='".$this->orde."' and a.porislr<>0 and 
                                                        coalesce(a.rifalt,'')=(CASE WHEN '".$tira1."'='' THEN coalesce(a.rifalt,'') else '".$tira1."' END ) and  
							a.numord=b.numord and                                                        
                                                        coalesce(a.rifalt,b.cedrif)=e.cedrif 	
                            														
							order by a.numord";
			//print $this->sql; exit;// and , d.numche
                                                        //b.ctaban=d.numcue left outer join tscheemi d on (b.numche = d.numche)
		...
		$this->sql2="update opfactur set rifalt='".$rifalt[0]."'
					   where numord='".$this->orde."' and monislr > 0";
		...
		function Header()
		{
			...
			$this->cell(260,5,"COMPROBANTE DE RETENCION DEL IMPUESTO SOBRE LA RENTA",0,0,'C');
			$this->ln(5);
			$this->setFont("Arial","",7);
			$this->cell(70,3,"");
			$this->multicell(120,3,"Reglamento Parcial de la ley de Impuesto Sobre la Renta en materia de Retenciones Dec 1808 6.0.36.203 de 12/05/1997",0,'C',0);
			$this->ln(5);
		...
		$tb=$this->bd->select($this->sql);
		 ...
		 $this->cell(65,5,substr($tb->fields["fecemi"],6,4).substr($tb->fields["fecemi"],3,2).str_pad($this->correlativo,8,"0",STR_PAD_LEFT));
                 //$this->cell(20,5,$this->fechasys);
                 $this->cell(20,5,$tb->fields["fecemi"]);
			//echo $this->fechasys;
		 ...
		 //$this->cell(30,5,$this->mes);
                 $this->cell(30,5,substr($tb->fields["fecemi"],3,2));
		 $this->ln(6);
		 $this->cell(20,5,"");
	...
	//	$this->cell(119,5,strtoupper($ben));
	 //   $this->cell(77,5,strtoupper($tira));
		...
		$this->cell(41,5,number_format($monret,2,',','.'),0,0,"R");
		$this->setFont("Arial","B",8);
		$this->line(15,190,80,190);
		$this->setxy(20,190);
		$this->cell(25,5,"Firma y Sello del Agente de Retención");
		$this->setxy(85,187);
		...
		$this->setxy(125,190);
		$this->cell(25,5,"Firma del Contribuyente");
		$this->line(120,190,165,190);
		$this->setxy(220,190);
		$this->cell(25,5,"Fecha de Entrega: ");
		$this->setxy(210,187);
		$this->cell(25,5,$this->diradm);
		$this->line(215,190,250,190);
		}
	}
?>
