...
class pdfreporte extends PDF_ADDS {
    ...
    function pdfreporte() {
        ...
        if ($this->status == "") {
            $this->condicion = " (A.STATUS='A' OR A.STATUS='I' OR A.STATUS='N')";
        }
        if ($this->status == "Todas") {
            $this->condicion = " (A.STATUS='A' OR A.STATUS='I' OR A.STATUS='N')";
        }
        if ($this->status == "Pagadas") {
            $this->condicion = " (A.STATUS='I')";
        }
        if ($this->status == "No Pagadas") {
            $this->condicion = " (A.STATUS='N')";
        }
        if ($this->status == "Anuladas") {
            $this->condicion = " (A.STATUS='A' )";
        }
        ...
        if ($this->codprodes == "") {
            ...
            A.CODMON,
            Z.NOMMON,
            (A.MONORD-A.MONRET-A.MONDES) AS NETO, a.valmon,
            (CASE WHEN A.STATUS='I' THEN 'Pagadas' WHEN A.STATUS='N' THEN 'En Finanazas' WHEN A.STATUS='A' THEN 'Anuladas' END) AS STAORD,
            B.NOMEXT,C.codpro, C.nrocei,E.telben as telpro, E.dirben  as dirpro, A.obsord, A.TIPCAU, b.afeprc, b.afecom, b.afecau, b.afedis
            FROM OPORDPAG A LEFT OUTER JOIN TSDESMON Z ON A.CODMON=Z.CODMON,CPDOCCAU B,OPBENEFI E left outer join  CAPROVEE C on e.cedrif=c.codpro
    ...
    function Header() {
        ...
        $this->SetXY(170, 3);
        $this->Cell(40, 10, 'Pagina ' . $this->PageNo() . ' de {nb}', 0, 0, 'R');
        $this->SetXY(175, 9);
    ...
    function formato() {
        ...
        $this->SetXY(167, 45 - $r);
        $this->SetFont("Arial", "B", 8);
        $this->Cell(10, 5, 'CÓDIGO POSTAL: 1071', 0, 0, 'L');
        ...
        $this->SetXY(10, 80);
        $this->SetFont("Arial", "B", 8);
        $this->Cell(200, 5, 'IMPUTACIÓN PRESUPUESTARIA (EN BOLÍVARES)', 0, 0, 'C');
        ...
        $this->SetXY(10, 135);
        $this->Cell(180, 5, 'CÓDIGOS CONTABLES (EN BOLÍVARES)', 0, 0, 'C');
        $this->SetXY(15, 140);
        ...
        $firmas = array(
            'ANALISTA FINANZAS' => ' ',
            'JEFE(A) FINANZAS' => ' ',
            'DIRECTOR(A) DE ADMÓN. Y FINANZAS' => ' ',
            'DIRECTOR(A) GENERAL EJECUTIVO' => ' ',
        );

        $grupos = array(
            195 => 'PARA USO DE LA DIRECCION DE ADMINISTRACION Y FINANZAS',
        );

        $this->getFirmas(4, $firmas, 8, 195, 13, $grupos);
    ...
    function Cuerpo() {
        ...
        //  $this->impt=0;
        while (!$tbgen->EOF) {
            $moneda = $tbgen->fields["nommon"];
            $valmon = $tbgen->fields["valmon"];
            $adicionapagina = true;
            if ($tbgen->fields["afeprc"] == 'N' and $tbgen->fields["afecom"] == 'N' and $tbgen->fields["afecau"] == 'N' and $tbgen->fields["afedis"] == 'N') {
                $sqltb = "select '' as CODPRE, '' AS NOMPRE, C.monord as MONCAU, C.MONDES, C.MONRET,c.VALMON
	  		 FROM opordpag C
            ...
            C.MONCAU,
            C.MONDES/A.VALMON AS MONDES,A.VALMON,
            C.MONRET/A.VALMON AS MONRET,A.VALMON
            FROM OPDETORD C,CPDEFTIT D,OPORDPAG A
            WHERE C.NUMORD='" . $tbgen->fields["numord"] . "' AND
            C.CODPRE=D.CODPRE AND 
            C.NUMORD = A.NUMORD
            ORDER BY C.CODPRE;";
            }
            ...
            $sqlret = "SELECT  RTRIM(B.NUMORD) AS ORDRET,B.CODTIP,C.DESTIP,C.PORRET,SUM(B.MONRET/A.VALMON) AS SUMRET
              FROM  OPRETORD B, OPTIPRET C,opordpag a
              WHERE B.NUMORD='" . $tbgen->fields["numord"] . "' AND
              RTRIM(B.CODTIP)=RTRIM(C.CODTIP) AND
              B.NUMORD=A.NUMORD
              GROUP BY
              RTRIM(B.NUMORD),
              B.CODTIP,
            ...
            $sqlcon = "SELECT
              ...
              B.CODCTA = C.CODCTA
              ORDER BY B.DEBCRE, B.CODCTA DESC;";
...
//ENCABEZADO
                    ...
                    $this->SetFont("Arial", "", 7);
                    if ($this->afeprc <> 'N' or $this->afecom <> 'N' or $this->afecau <> 'N' or $this->afedis <> 'N') {
                        //print $valmon;
                        //
            
            $this->MultiCell(120, 2, montoescrito($tbgen->fields["neto"] / $valmon, $this->bd, $moneda));
                        $this->SetXY(170, 66 - $this->r);
                        $this->SetFont("Arial", "B", 8);


                        $this->Cell(45, 5, number_format($tbgen->fields["neto"] / $valmon, 2, ",", "."), 0, 0, "L");
                    }
                    ...
                    $this->MultiCellMax(170, 3, $tbgen->fields["desord"], 0, 'J', 0, 3); //concepto
                    $this->SetXY(10, 190);
                    $this->SetFont("Arial", "B", 8);
                ...
                if (($cuantasimp < $impxpag) && (!$tb->EOF)) {
                    ...
                    $ver2+=$ver;
                    $this->SetXY(60, $yimp);
                    $tota+=$tb->fields["moncau"] / $tb->fields["valmon"];
                    if ($tb->fields["codpre"] != '')
                        $totaimp+=$tb->fields["moncau"];
                    $tota2+=$tb->fields["moncau"] / $tb->fields["valmon"];
                    $monret+=$tb->fields["monret"];
                    $mondes+=$tb->fields["mondes"];
                    $monnet+= ( $tb->fields["mdoncau"] - $tb->fields["monret"] - $tb->fields["mondes"]);
                    $this->SetXY(90, $yimp + 1, 9);
            ...
            $this->Cell(45, 5, number_format($totaimp, 2, ",", "."), 0, 0, "R");
            $this->SetXY(153, 225);
            ...
            if ($this->afeprc == 'N' and $this->afecom == 'N' and $this->afecau == 'N' and $this->afedis == 'N') {
                //print ($tota - $imp - $mondes);
                $this->MultiCell(120, 2, montoescrito(($tota - $imp - $mondes), $this->bd, $moneda));
                $this->SetXY(170, 66 - $this->r);
                $this->SetFont("Arial", "B", 10);
