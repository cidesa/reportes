<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$pos1=strpos($this->benfac,"?");
                        $ben1=substr($this->benfac,$pos1+1,strlen($this->benfac));
                        $tira1=substr($this->benfac,0,$pos1);



				$this->sql="select distinct a.numord,a.rifalt,b.cedrif,e.nomben,b.fecemi,to_char(a.fecfac,'dd/mm/yyyy') as fecfac,a.numfac,
							a.numctr,a.tiptra,a.facafe,
							a.totfac,a.exeiva,a.basimp,a.poriva,a.moniva,a.monret,a.rifalt
							from
							opfactur a,
							opordpag b,
							...
							a.numord='".$this->orde."' and
                                                        coalesce(a.rifalt,'')=(CASE WHEN '".$tira1."'='' THEN coalesce(a.rifalt,'') else '".$tira1."' END ) and
							a.numord=b.numord and
							a.numord=c.numord and
							c.codtip=d.codtip and
							coalesce(a.rifalt,b.cedrif)=e.cedrif and							
							a.basimp>0.00 and
							a.exeiva>=0.00 and a.moniva>0.00 and a.monret>0.00
							order by a.numord";
//print $this->sql;exit;
	...
	//print $this->sql;
		...
		function br()
		{
			...
			$this->ano=date('Y');
			$this->mes=date('m');
			$pos2=strpos($this->benfac,"?");
                        $ben2=substr($this->benfac,$pos2+1,strlen($this->benfac));
                        $tira2=substr($this->benfac,0,$pos2);
		//---------------------
	/*	$this->sql1="select comret,feccomret, to_char(feccomret,'dd/mm/yyyy') as feccomretc,status
					from opordpag where numord='".$this->orde."'";                        */
		$this->sql1="select a.comret,a.feccomret, to_char(a.feccomret,'dd/mm/yyyy') as feccomretc,a.status
				from opordpag a 
				where 
				a.numord='".$this->orde."'";
//print $this->sql1;exit;
		...
		$this->sql2="update opfactur set rifalt='".$this->benalt."'
					   where numord='".$this->orde."' and poriva<>0";   
		...
		if (strtoupper($this->corr)=="N")//NO
		{
		   if($this->cor==null)
		   {
			...
			{ // si el de la bd esta vacio
				...
				$this->sql4="update opordpag set comret='".$this->correlativo."', feccomret=to_date('".$this->fechasys."','dd/mm/yyyy')
						where numord='".$this->orde."'";
				/*$this->sql4="update opfactur set comret='".$this->correlativo."', feccomret=to_date('".$this->fechasys."','dd/mm/yyyy')
                                                where   
						numord='".$this->orde."' and
		                                rifalt='".$this->benfac."' 
                		                ";*/
		   ...
		   {	
			...
			//$this->sql4="update opordpag set comret='".$this->correlativo."'
						//where numord='".$this->orde."'";
 			$this->sql4="update opordpag set comret='".$this->correlativo."', feccomret=to_date('".$this->fechasys."','dd/mm/yyyy')
                          where numord='".$this->orde."'";  
			/*$this->sql4="update opfactur set comret='".$this->correlativo."', feccomret=to_date('".$this->fechasys."','dd/mm/yyyy')
		...
		{ 
			...
			$this->sql5="update opordpag set feccomret=to_date('".$this->fechasys."','dd/mm/yyyy')
						where numord='".$this->orde."'";			
			/*$this->sql5="update opfactur set comret='".$this->correlativo."', feccomret=to_date('".$this->fechasys."','dd/mm/yyyy')
                                     where
                                        numord='".$this->orde."' and
                                        rifalt='".$this->benfac."'"; */
		...
		function Header()
		{
			...
			$this->Rect(210,$this->GetY()+15,60,12);
			$this->Rect(10,$this->GetY()+10,195,13);
			$this->ln(1);
			$this->cell(1,5,"");
			...
			$this->cell(15,5,"de Débito");
			$this->cell(14,5,"de Crédito");
...
//		$this->cell(80,5,strtoupper($tira));
		...
		$this->cell(18,5,$tb->fields["numfac"],0,0,"C");
		$this->cell(17,5,$tb->fields["numctr"],0,0,"C");
		$this->cell(15,5,$tb->fields[" "],0,0,"C");
		$this->cell(18,5,$tb->fields[" "],0,0,"C");
	...
	//	$this->cell(25,5,"Elaborado por:  ");
		$this->setxy(210,187);
	//	$this->cell(25,5,'Coordinador(a) de Administracion y Servicio');
