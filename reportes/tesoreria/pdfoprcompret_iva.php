...
class pdfreporte extends fpdf {
    ...
    function pdfreporte() {
        ...
        $this->sql = "select a.numord,to_char(b.fecemi,'dd/mm/yyyy') as fecemi, a.rifalt as cedrif,c.nomben, c.nitben, c.dirben, c.telben,to_char(a.fecfac,'dd/mm/yyyy') as fecfac, a.numfac,
							a.numctr,a.tiptra,a.facafe,
							a.totfac,a.exeiva,a.basimp,a.poriva,a.moniva,a.monret, d.numche
							from opbenefi c,opfactur a,opordpag b 
							left outer join tscheemi d on (b.numche = d.numche)
							where
                                                        a.rifalt=c.cedrif and
    ...
    function br() {
        if ($this->feccomp == "") {
            ...
            $this->ano = $aux[2];
            $this->mes = $aux[1];
            $this->sql44 = "update opfactur set feccomret=to_date('" . $this->fechasys . "','dd/mm/yyyy')	where numord='" . $this->orde . "' and rifalt=(case when '" . trim($this->ben) . "'='' then (select cedrif from opordpag where numord='" . $this->orde . "') else '$this->ben' end)";
            $tb44 = $this->bd->actualizar($this->sql44);
        }
        if ($this->numcomp <> "") {
            $this->sql45 = "update opfactur set comret='" . $this->numcomp . "' where numord='" . $this->orde . "' and rifalt=(case when '" . trim($this->ben) . "'='' then (select cedrif from opordpag where numord='" . $this->orde . "') else '$this->ben' end)";
            $tb44 = $this->bd->actualizar($this->sql45);
        }

        $this->sql1 = "select Distinct a.comret,a.feccomret, to_char(a.feccomret,'dd/mm/yyyy') as feccomretc, to_char(a.feccomret,'mm') as mes,to_char(a.feccomret,'yyyy') as ano
...
//		}
    ...
    function Footer() {
        ...
        $this->SetXY(15, 200);
        $this->Line(25, $this->GetY(), 80, $this->GetY());
        $this->MultiCell(70, 5, $_POST["responsable"] . "\nFirma y Sello \nAgente de Retención", 0, 'C');
        $this->SetXY(211 - 20, 198);
        $this->cell(100, 3, "Recibido por: _______________________________", 0, 'C');
        $this->ln(4);
        $this->SetX(227 - 20);
        $this->cell(100, 3, "C.I: _______________________________", 0, 'C');
        $this->ln(4);
        $this->SetX(215 - 20);
        $this->cell(100, 3, "        Fecha: _______________________________", 0, 'C');
        $this->ln(4);
        $this->SetX(216 - 20);
    ...
    function Header() {
        ...
        $this->Image("../../img/logo_1.jpg", 10, 10, 40);
        $this->SetXY(50, 10);
        $this->Cell(10, 3, 'REPUBLICA BOLIVARIANA DE VENEZUELA');
        $this->ln();
        $this->SetX(50);
        $this->Cell(10, 3, 'Ministerio del Poder Popular para la Educación Universitaria');
        $this->ln();
        $this->SetX(50);
        $this->Cell(10, 3, 'FUNDACION FAMES');
        $this->ln();
        $this->SetX(50);
        $this->Cell(10, 3, 'GESTION ADMINISTRATIVA Y FINANCIERA');
...
#		 $this->cell(65,5,$this->ano."  -  ".$_POST["co"]);
            $this->cell(63, 5, $this->ano . " - " . $this->mes . " - " . str_pad($this->correlativo, 8, "0", STR_PAD_LEFT));
            //$this->cell(20,5,$_POST["fechaentrega"]);fecha de retencion
            $this->cell(20, 5, $this->fechasys); //$this->cell(20,5,$tb2->fields["fecemi"]);
            #$this->cell(20,5,$tb2->fields["fecret"]);

            $puntoy = $this->GetY();
            $this->SetXY(226, 79);
            $this->Cell(45, 5, '', 0, 0, 'C'); //$this->Cell(45,5,$this->fechasys,0,0,'C');
            #$this->SetXY(226,94);
            ...
            $this->cell(130, 5, strtoupper($this->ag));
            $this->ln(3);
            ...
            $y = $this->getY();
            $this->setx(28);
            $this->multicell(95, 3, strtoupper($this->dire));
            $this->SetXY(145, $y);
            $this->cell(115, 5, ''); //TLF
...
//		 }
            ...
            $this->multicell(14, 5, $tb->fields["numfac"], 1);
            $this->SetXY(75, 117);
            $this->multicell(14, 5, $tb->fields["numctr"], 1);
            $this->SetXY(89, 117);
...
#		 $this->cell(65,5,$this->ano."  -  ".$_POST["co"]);
                    $this->cell(65, 5, $this->ano . "  -  " . $this->mes . "  -  " . str_pad($this->correlativo, 8, "0", STR_PAD_LEFT));
                    //$this->cell(20,5,$_POST["fechaentrega"]);fecha de retencion
                    $this->cell(20, 5, $this->Cell(45, 5, $this->fechasys, 0, 0, 'C'));
                    #$this->cell(20,5,$tb2->fields["fecret"]);
                    ...
                    $this->multicell(95, 3, '');
                    $this->SetXY(145, $y + 5);
                    $this->cell(115, 5, strtoupper($this->dire), 0, 0, 'L'); //TLF
                    //$this->cell(115,5,strtoupper($this->dire));
                    $this->ln(20);
//		 if ($this->benalt<>" ")
//		 {
