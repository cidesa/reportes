...
<tr>
              ...
              <td colspan="2" width='100'> <div align="left">
                ...
                $sql="SELECT distinct(a.numord) as numord
               ...
               LlenarTextoSql($sql,"numord",$bd);
                ...
                <input type="button" name="ButtonOrd" value="..." onClick="catalogo1('orde');">
                </div></td>
              <td>
            </tr>  

<!--	 <tr>
              <td class="form_label_01"> <div align="left"><strong>Numero de Factura:</strong></div></td>
              <td colspan="2"> <div align="left">
                  <?

                                $tb=$bd->select("SELECT distinct(a.numord),a.numfac
                                                                from opfactur a, opordpag b, opretord c, tsrepret d
                                                                where
                                                                a.numord=b.numord and
                                                                b.numord=c.numord and
                                                                ...
                                                                order by a.numord");
                          ?>
                  <select name="fac" class="breadcrumb" id="fac">
                    <?
                                        while(!$tb->EOF)
                                        {
                                  ?>
                    <option value="<?  print $tb->fields["numord"]."?".$tb->fields["numfac"];?>"><? print $tb->fields["numord"]."  -  ". $tb->fields["numfac"];?></option>
                    <?
                                        $tb->MoveNext();
                                        }
                                ?>
 ...
 <tr>
              ...
              <td colspan="2">
                <?
                                ...
                                $tb2=$bd->select("select distinct rtrim(a.rifalt) as cedrif, rtrim(b.nomben) as nomben 
						from opfactur a, opbenefi b
						where a.rifalt=b.cedrif
						order by nomben
						");
                          ?>
                <select name="benfac" class="breadcrumb" id="benfac">
                                 <option VALUE=""> </option>
                  <?
                                        while(!$tb2->EOF)
                                        {
	...
	<!--	  <option value="<? print strtoupper($tb2->fields["cedrif"]);?>"><strong><? print strtoupper(str_pad($tb2->fields["cedrif"],15," ",STR_PAD_RIGHT));?></strong></option>-->
...
{
 	mysql="SELECT distinct(a.numord) as numord from opfactur a, opordpag b, opretord c, tsrepret d where a.numord=b.numord and b.numord=c.numord and d.codrep='001' and c.codtip=d.codret order by a.numord";
