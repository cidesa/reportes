	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->rifconhas=trim($_POST["rifconhas"]);
				...
				$this->sql="select a.refing, to_char(a.fecing,'dd/mm/yyyy') as fecing, a.rifcon, b.nomcon, a.moning, a.desing,
                                    case when b.tipcon='N' then 'Natural' when b.tipcon='J' then 'Jurídico' when b.tipcon='O' then 'Oficial' end as tipcon
							from cireging a, ciconrep b
							where
							a.rifcon=b.rifcon and
							(a.rifcon) >= ('".$this->rifcondes."') and
							(a.rifcon) <= ('".$this->rifconhas."') and
							(a.fecing) >= to_date('".$this->fecregdes."','dd/mm/yyyy') and
							(a.fecing) <= to_date('".$this->fecreghas."','dd/mm/yyyy')
							order by a.rifcon";
		...
		{
				$this->titulos2[0]="CONCEPTO";
				$this->titulos2[1]="";
				$this->titulos2[2]="PLANILLA";
				$this->titulos2[3]="FECHA";
				$this->titulos2[4]="RECAUDADO";
		}
			...
			while(!$tb->EOF)
			{
				...
				$this->cell($this->anchos2[4],6,number_format($acum,'2','.',','),0,0,'R');
				 ...
				 $this->setFont("Arial","",8);
                                 $this->SetX(10+$this->anchos2[0]);
				 $this->cell($this->anchos2[1],4,$tb->fields["nrocon"]);
				 $this->cell($this->anchos2[2],4,$tb->fields["refing"]);
				 $this->cell($this->anchos2[3],4,$tb->fields["fecing"]);
				 $this->cell($this->anchos2[4],4,number_format($tb->fields["moning"],'2','.',','),0,0,'R');
                                 $this->SetX(10);
                                 $this->Multicell($this->anchos2[0],4,$tb->fields["desing"]);
				 $this->ln();
				...
				$this->cell($this->anchos2[4],6,number_format($acum,'2','.',','),0,0,'R');
                                $this->ln();
                                $this->SetX($this->anchos2[0]+$this->anchos2[1]+$this->anchos2[2]+10);
				$this->cell($this->anchos2[3],6,'Total PLanillas    ',0,0,'R');
				$this->cell($this->anchos2[4],6,number_format($cuantos,'2','.',','),0,0,'R');
