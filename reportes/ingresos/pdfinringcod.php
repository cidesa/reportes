...
class pdfreporte extends fpdf
{
	...
	function pdfreporte()
	{
	 ...
	 $this->sql="select distinct b.codpre, to_char(a.fecing,'dd/mm/yyyy') as fecingstr, a.refing, a.desing, a.moning, 0 AS mondes, a.montot, a.codtip, a.fecing
	 		    from
	...
	function Header()
	{
	 ...
	 $this->ln(5);
	 $this->SetWidths(array(25,100,25,50,30,30));
	 $this->SetBorder(true);
	 $this->Setaligns(array('C','C','C','C','C','C'));
	 $this->Row(array('Referencia de Ingreso','Descripcion','Fecha','Tipo','Monto','Monto Total'));
	...
	function Cuerpo()
	{
		while(!$this->tb->EOF)
		{
			...
			while(!$this->tb->EOF and $ref==$cod)
			{
				...
				$this->Row(array(trim($this->tb->fields["refing"]),trim($this->tb->fields["desing"]),$this->tb->fields["fecingstr"],$nomtip,H::FormatoMonto($this->tb->fields["moning"]),H::FormatoMonto($this->tb->fields["montot"])));
