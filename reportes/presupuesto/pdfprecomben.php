	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
			...
			$this->benhasta=$_POST["benhas"];
                        $this->estadocompromiso=$_POST["estadocompromiso"];
                        $this->resumido=$_POST["resumido"];
			$tipocompromiso="";
			if ($this->estadocompromiso=="Sin Causar")
                        {
                            $tipocompromiso=" A.SALCAU=0";
                        }
                        else
                        {
                            if ($this->estadocompromiso=="Causado No Pagado")
                            {
                                $tipocompromiso="  A.SALCAU>0 AND A.SALPAG=0";
                            }
                            else
                            {
                               if ($this->estadocompromiso=="Causado y Pagado")
                               {
                                  $tipocompromiso="  A.SALCAU>0 AND A.SALPAG>0";
                               }
                               else
                               {
                                  $tipocompromiso="  A.MONCOM>=0 ";
                               }
                            }
                    ...
                    if($this->stacom=="Todos")
                        {
                            $statuscom=" (A.StaCom='A' or A.StaCom='N') And ";
                        }
                        else
                           if($this->stacom=="Activo")
                           {
                               $statuscom=" (A.StaCom='A' or (A.StaCom='N' and A.FecAnu>To_Date('".$this->feccom2."','dd/mm/yyyy'))) AND ";
                           }
                           else
                           {
                              $statuscom=" (A.StaCom='N' and A.FecAnu<= To_Date('".$this->feccom2."','dd/mm/yyyy')) AND ";
                           }
                    if ($this->resumido=="Resumido")
                    {
			...
			$this->sql="SELECT A.refcom as referencia,
							...
							sum(b.monimp) as imputado,
							sum(b.moncau)  as moncau,
							sum(b.monpag) as monpag,
							sum(b.monaju) as ajuste,
							sum(b.monimp-b.monaju) as mon_aju,
							D.NOMBEN as beneficiario,
							D.cedrif as cedben
						FROM CPCOMPRO A, OPBENEFI D,CPIMPCOM B
						WHERE
                			...
                			A.REFCOM<='".$this->numcom2."' AND
               				...
               				A.TIPCOM >= '".$this->tipcom1."' AND
                			A.TIPCOM <= '".$this->tipcom2."' AND
					trim(A.CEDRIF) >=trim('".$this->bendesde."') AND 
                         		trim(A.CEDRIF) <=trim('".$this->benhasta."') AND 
                                        D.CEDRIF=A.CEDRIF AND 
                                        a.refcom=b.refcom AND ".$statuscom.$tipocompromiso.
                             " group by A.refcom,A.tipcom ,A.feccom,A.CEDRIF,
                                        (CASE WHEN A.STACOM='A' THEN 'Activo' WHEN A.STACOM='N' THEN 'Anulado' ELSE '' END),A.FECANU,
					a.stacom, substr(RTRIM(A.descom),1,50),d.cedrif,d.nomben
                              ORDER BY D.NomBen, D.cedrif,A.FECCOM,A.REFCOM,A.TIPCOM ";
                        // print $this->sql;exit;

                    }
                    if ($this->resumido=="Detallado")
                    {
			...
			$this->sql="SELECT A.refcom as referencia,
					...
					trim(A.CEDRIF) >=trim('".$this->bendesde."') AND 
                			trim(A.CEDRIF) <=trim('".$this->benhasta."') AND
               				B.CODPRE LIKE RTRIM('".$this->comodin."') AND ".$statuscom.$tipocompromiso.
                             " ORDER BY D.cedrif,A.REFCOM,A.FECCOM,B.CODPRE,A.TIPCOM ";
		// print $this->sql;exit;

                        }
                        if ($this->resumido=="General")
                        {
		...
		function llenartitulosmaestro()
		{
				...
				$this->titulos[3]="DESCRIPCION";
                                $this->titulos[4]="COMPROMETIDO";
				$this->titulos[5]="REGULARIZACIÓN";
				$this->titulos[6]="MTO REGULARIZADO";
				...
				$this->titulos[3]="";
                                $this->titulos[4]="COMPROMETIDO";
				$this->titulos[5]="AJUSTADO";
				$this->titulos[6]="MONTO AJUSTADO";
				$this->titulos[7]="CAUSADO";
                                $this->titulos[8]="PAGADO";
				$this->anchos[0]=20;
				$this->anchos[1]=10;
		...
		function Header()
		{
			...
			$ubica = count($parte)-2;
			$this->cab->poner_cabecera($this,$_POST["titulo"]." (".$this->estadocompromiso.")","l","s",$parte[$ubica]);
                ...
                function General()
		{
			...
			{
			        $this->setFont("Arial","B",8);
                                if ($ben!=$tb->fields["cedrif"])
                                {
                                    //$this->SetXY(10,$this->GetY());
                                    //$this->cell(100,5,$tb->fields["beneficiario"]);
                                    $ben=$tb->fields["cedrif"];
                                    $nomben=$tb->fields["beneficiario"];
                                    //$this->ln();
                                }
				...
				$b=$this->bd->select("SELECT coalesce(SUM(B.MONIMP),0) as SALDOPAG
        								...
        								(B.staIMP='A' Or (b.staimp='N' And A.FecAnu>to_date('".$this->feccom2."','dd/mm/yyyy'))) ");
				if(!$b->EOF)
				{
				...
				$tb->MoveNext();
                                ...
                                if ($ben!=$tb->fields["cedrif"])
                                {
                                    ...
                                    $this->SetXY(10,$this->GetY());
                                    $this->setWidths(array(20,10,20,80,25,25,25,25,25));
			            $this->setAligns(array("L","L","L","L","R","R","R","R","R"));
                                    $this->Rowm(array($ben,
                                    "",
				    "",
                                    $nomben,
				    H::FormatoMonto($acumBen1),
                                    H::FormatoMonto($acumBen2),
                                    H::FormatoMonto($acumBen3),
                                    H::FormatoMonto($acumBen4),
                                    H::FormatoMonto($acumBen5)));
                                    $this->setFont("Arial","",8);
                                    $acumBen1=0;
                                    $acumBen2=0;
                                    $acumBen3=0;
				...
				//Totales
				 ...
				 $this->ln(6);
				 $this->line(100,$this->getY(),270,$this->getY());
				 $this->Rowm(array("TOTALES",
                                    "",
				    "",
                                    "",
				    H::FormatoMonto($acum1),
                                    H::FormatoMonto($acum2),
                                    H::FormatoMonto($acum3),
                                    H::FormatoMonto($acum4),
                                    H::FormatoMonto($acum5)));
				 $this->ln(10);
				 ...
				 $this->ln(4);
				 $this->cell(60,10,'TOTAL GENERAL DE COMPROMISOS       	 						'.number_format(($totcaucom+$totpagcom+$acum3),2,'.',','));
				//
		}
...
//				 $this->cell($this->anchos[4],10,$tb->fields["beneficiario"]);
				...
				$acumBen5=$acumBen5+(abs($saldpag-$ajustepag));
                                ...
                                $this->setWidths(array(20,10,20,80,25,25,25,25,25));
			        $this->setAligns(array("L","L","L","L","R","R","R","R","R"));
                                $this->Rowm(array($tb->fields["referencia"],
                                $tb->fields["tipo"],
				$tb->fields["fecha"],
                                $tb->fields["descripcion"],
				H::FormatoMonto($tb->fields["imputado"]),
                                H::FormatoMonto($saldoAjustado),
                                H::FormatoMonto($tb->fields["imputado"]-$saldoajustado),
                                H::FormatoMonto($saldcau-$ajucau),
                                H::FormatoMonto($saldpag-$ajustepag)));
                ...
                function Detallado()
		{
			...
			if(!$tb2->EOF)
			{
				...
				$ben=trim($tb2->fields["cedben"]);
				 ...
				 $this->cell($this->anchos[4],4,$tb2->fields["beneficiario"]);
 				 $this->ln(5);
				 $this->setFont("Arial","B",8);
				 $this->cell($this->anchos[0],4,$tb2->fields["referencia"]);
				 if(strtoupper($tb2->fields["stacom"])=="A")
				 	$status="Activo";
				 else
				 	$status="Anulado";
				 $this->cell($this->anchos[1],4,$status);
				 $this->cell($this->anchos[2],4,$tb2->fields["tipo"]);
				 $this->cell($this->anchos[3],4,$tb2->fields["fecha"]);
				 $this->multicell($this->anchos[4],4,$tb2->fields["descripcion"]);
			}
			while(!$tb->EOF)
              ...
              ///
				...
				if ($this->getY()>170 )
				 ...
				 if(strtoupper($tb2->fields["stacom"])=="A")
				 	...
				 	$status="Anulado";
				 $this->cell($this->anchos[1],4,$status);
				 $this->cell($this->anchos[2],4,$tb->fields["tipo"]);
				 $this->cell($this->anchos[3],4,$tb->fields["fecha"]);
				 $this->multicell($this->anchos[4],4,$tb->fields["descripcion"]);
//				 $this->cell($this->anchos[4],10,$tb->fields["beneficiario"]);
		        }
				...
				$this->cell($this->anchos2[2],4,number_format($tb->fields["imputado"],2,'.',','));
				$acum1=$acum1+$tb->fields["imputado"];
				$acumBen1=$acumBen1+$tb->fields["imputado"];
				$t=$this->bd->select("SELECT coalesce(SUM(B.MONAJU),0) as SALDOAJU
										FROM CPAJUSTE A,CPMOVAJU B,CPDOCAJU C
         								...
         								A.TIPAJU=C.TIPAJU AND
         								C.REFIER='C' AND
         								B.CODPRE=RPAD('".$tb->fields["codigo"]."',50,' ')");
				if(!$t->EOF)
				{
					$saldoAjustado=$t->fields["saldoaju"];
				}
				$this->cell($this->anchos2[3],4,number_format($saldoAjustado,2,'.',','));
				$acum2=$acum2+($saldoAjustado);
				$acumBen2=$acumBen2+($saldoAjustado);
				$this->cell($this->anchos2[4],4,number_format(($tb->fields["imputado"]-$saldoAjustado),2,'.',','));
				$acum3=$acum3+($tb->fields["imputado"]-$saldoAjustado);
				$acumBen3=$acumBen3+($tb->fields["imputado"]-$saldoAjustado);
				$p=$this->bd->select("SELECT coalesce(SUM(B.MONIMP),0) as SALDOCAU
										FROM CPCAUSAD A,CPIMPCAU B
				...
				$this->cell($this->anchos2[5],4,number_format(abs($saldcau-$ajucau),2,'.',','));
				$acum4=$acum4+(abs($saldcau-$ajucau));
				$acumBen4=$acumBen4+(abs($saldcau-$ajucau));
				$b=$this->bd->select("SELECT coalesce(SUM(B.MONIMP),0) as SALDOPAG
										FROM CPPAGOS A,CPIMPPAG B
         								...
         								A.FECPAG<=to_date('".$this->feccom2."','dd/mm/yyyy') AND
         								A.FECPAG>=to_date('".$this->feccom1."','dd/mm/yyyy') AND
        								(B.staIMP='A' Or (b.staimp='N' And A.FecAnu>to_date('".$this->feccom2."','dd/mm/yyyy'))) And B.CODPRE=('".$tb->fields["codigo"]."')");
				if(!$b->EOF)
				{
					$saldpag=$b->fields["saldopag"];
				}
				...
				//ojo
				$this->cell($this->anchos2[6],4,number_format(abs($saldpag-$ajustepag),2,'.',','));
				$this->setX($x);
				$this->multicell($this->anchos2[1]-10,4,$tb->fields["nombre"]);
				$acum5=$acum5+(abs($saldpag-$ajustepag));
				$acumBen5=$acumBen5+(abs($saldpag-$ajustepag));
				$tb->MoveNext();
			}
