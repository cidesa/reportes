	...
	class pdfreporte extends fpdf
	{
		...
		function llenartitulosmaestro()
		{
				...
				$this->titulos[7]="ASIGNACION INICIAL";
                                $this->titulos[8]="MODIFICACIONES";
				$this->titulos[9]="PRE-COMPROMISO";
				$this->titulos[10]="DISPONIBILIDAD";
				$this->titulos[11]="";
				$this->titulos[12]="DIRECCION DE PLANIFICACION Y PRESUPUESTO";
		}
  ...
  function llenaranchos()
	{
		...
		function Header()
		{
			...
			$this->ln();
		    $this->SetWidths(array($this->anchos[0],$this->anchos[1],$this->anchos[2],$this->anchos[3],$this->anchos[4],$this->anchos[5]+$this->anchos[6]-2,$this->anchos[7],$this->anchos[8],$this->anchos[9],$this->anchos[9]));
			$this->SetAligns(array("C","C","C","C","C","C","C","C","C","C"));
		    $this->SetBorder(1);
			$this->Row(array($this->titulos[0],$this->titulos[1],$this->titulos[2],$this->titulos[3],$this->titulos[4],$this->titulos[5],$this->titulos[7],$this->titulos[8],$this->titulos[9],$this->titulos[10]));
		...
		function Cuerpo()
		{
			...
			$this->SetAligns(array("C","C","C","C","C","L","R","R","R","R"));
		    $this->SetBorder(1);
			$this->Row(array($aux[2],$aux[3],$aux[4],$aux[5],$aux[6],$dato["desprc"],H::FormatoMonto($dato["monasi"]),H::FormatoMonto($dato["monadi"]+$dato["montra"]-$dato["mondim"]-$dato["montrn"]),H::FormatoMonto($dato["monimp"]),H::FormatoMonto(($dato["monasi"]+$dato["monadi"]+$dato["montra"]-$dato["mondim"]-$dato["montrn"])-$dato["monimp"])));
			$acum1+=$dato["monasi"];
                        $acum0+=$dato["montra"]+$dato["monadi"]-$dato["montrn"]-$dato["mondim"];
			$acum2+=$dato["monimp"];
			$acum3+=($dato["monasi"]+$dato["montra"]+$dato["monadi"]-$dato["montrn"]-$dato["mondim"])-$dato["monimp"];
			}
	        $this->SetWidths(array($this->anchos[0]+$this->anchos[1]+$this->anchos[2]+$this->anchos[3]+$this->anchos[4]+$this->anchos[5]+$this->anchos[6]-2,$this->anchos[7],$this->anchos[8],$this->anchos[9],$this->anchos[9]));
			$this->SetAligns(array("R","R","R","R","R"));
		    $this->SetBorder(1);
			$this->Row(array("TOTALES",H::FormatoMonto($acum1),H::FormatoMonto($acum0),H::FormatoMonto($acum2),H::FormatoMonto($acum3)));
			...
			$this->Row(array("DIRECCIÓN DE PLANIFICACION Y PRESUPUESTO"));
