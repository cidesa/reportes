...
// print '<pre>'; print $this->sql;exit;
		...
		function llenartitulosmaestro()
		{
				...
				$this->titulos[10]="%";
				$this->titulos[11]="Disponibilidad Presupuestaria";
		...
		function Header()
		{
			...
			$this->setWidths(array(42,42,30,28,28,26,17,32,20,28,17,28));
                        //$this->setWidths(array($this->anchos[0],$this->anchos[1],$this->anchos[2],$this->anchos[3],$this->anchos[4],$this->anchos[5],$this->anchos[6],$this->anchos[7],$this->anchos[8],$this->anchos[9],$this->anchos[10],$this->anchos[11]));
			//$this->setBorder(1);
...
//			$this->bd->actualizar('SET search_path TO "SIMA055"');
			...
			while(!$tb->EOF)
			{
                ...
                $this->SetX(90);
				$this->cell($this->anchos[2],4,H::FormatoMonto($tb->fields["asignacion"]),0,0,'R');
				$this->cell($this->anchos[3],4,H::FormatoMonto($tb->fields["modificado"]),0,0,'R');
				$this->cell($this->anchos[4],4,H::FormatoMonto($tb->fields["asignacion"]+$tb->fields["modificado"]),0,0,'R');
				$this->cell($this->anchos[5],4,H::FormatoMonto($tb->fields["compromiso"]),0,0,'R');
				$this->cell($this->anchos[6],4,H::FormatoMonto(($tb->fields["compromiso"]*100)/($tb->fields["modificado"]+$tb->fields["asignacion"])),0,0,'R');
				$this->cell($this->anchos[7],4,H::FormatoMonto($tb->fields["causado"]),0,0,'R');
				$this->cell(20,4,H::FormatoMonto(($tb->fields["causado"]*100)/$tb->fields["compromiso"]),0,0,'R');
				$this->cell(30,4,H::FormatoMonto($tb->fields["pagado"]),0,0,'R');
				$this->cell(20,4,H::FormatoMonto(($tb->fields["pagado"]*100)/$tb->fields["causado"]),0,0,'R');
				$this->cell(30,4,H::FormatoMonto(($tb->fields["modificado"]+$tb->fields["asignacion"])-$tb->fields["compromiso"]),0,0,'R');
			...
			$this->cell(81,7,"TOTAL GENERAL",0,0,'R');
			$this->cell($this->anchos[2],7,H::FormatoMonto($totalasi),0,0,'R');
			$this->cell($this->anchos[3],7,H::FormatoMonto($totalmod),0,0,'R');
			$this->cell($this->anchos[4],7,H::FormatoMonto($totalasi+$totalmod),0,0,'R');
			$this->cell($this->anchos[5],7,H::FormatoMonto($totalcom),0,0,'R');
			$this->cell($this->anchos[6],7,H::FormatoMonto(($totalcom*100)/($totalasi+$totalmod)),0,0,'R');
			$this->cell($this->anchos[7],7,H::FormatoMonto($totalcau),0,0,'R');
			$this->cell(20,7,H::FormatoMonto(($totalcau*100)/$totalcom),0,0,'R');
			$this->cell(30,7,H::FormatoMonto($totalpag),0,0,'R');
			$this->cell(20,7,H::FormatoMonto(($totalpag*100)/$totalcau),0,0,'R');
			$this->cell(30,7,H::FormatoMonto(($totalasi+$totalmod)-$totalcom),0,0,'R');
