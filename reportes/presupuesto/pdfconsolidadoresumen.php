<?
	...
	function Header()
	{
	 ...
	 $this->ln(4);
	  $this->setFont("Arial","B",9);
	 $this->cell(20,6,$this->titulos[2]);  //Fecha
	 $this->cell(20,6,$this->titulos[3]);	//Tipo
	 $this->cell(30,6,$this->titulos[4]);   //precompr
	 $this->cell(30,6,$this->titulos[5]);	//comprom
	 $this->cell(30,6,$this->titulos[6]);	//causado
	 $this->cell(30,6,$this->titulos[7]);	//pagado
	 //$this->cell(70,6,$this->titulos[1]);	//descripcion
	 $this->cell(50,6,$this->titulos[13]);	//benefi
	 $this->cell(30,6,$this->titulos[10]);	//monto
	 $this->cell(30,6,$this->titulos[9]);	//Saldo
	 $this->ln();
	 $this->Line($this->GetX(),$this->GetY()+1,$this->GetX()+260,$this->GetY()+1);
	 $this->ln(6);
	 $this->SetWidths(array(20,15,30,30,30,30,40,30,30));
	...
	$this->SetTextColor(0,0,0);
     foreach($this->arrp as $datos)
	 {
	   ...
	   $this->cell(71,6,$datos["codpreasiini"]);
	   ...
	   $this->setFont("Arial","B",9);
	   $this->SetXY(130,50);
	   $this->cell(90,5,'ASIG.INICIAL:      '.H::FormatoMonto($asignacion[0][0]),0,0,'R');
	   $this->ln(13);
	...
	if($entro==false){
	$this->cell(30,6,H::FormatoMonto($asignacion[0][0]));
	}else if($entro==true){
	$this->cell(30,6,H::FormatoMonto($disponible));
	}
	$this->setFont("Arial","B",9);
	$this->SetX(15);
	$this->cell(60,6,$datos["codpreasiini"]);
	$this->SetX(87);
	$this->multicell(90,6,$datos["nompreasiini"]);
	$this->SetTextColor(0,0,0);
   ...
   //H::PrintR($cf_dismin);
	...
	$this->Cell(85,3,'TOTAL COMPROMETIDO:    '.H::FormatoMonto($cf_totcompro),0,0,'J');
	$this->Cell(85,3,'TOTAL AJUSTES COMPROMISOS:    '.H::FormatoMonto($cf_ajucom[0][0]),0,0,'J');
	$this->Cell(85,3,'TOTAL COMPR. AJUSTADOS:    '.H::FormatoMonto($cf_comaju),0,0,'J');
	$this->Ln(6);
	$this->SetX(10);
	$this->Cell(85,3,'TOTAL CAUSADO:    '.H::FormatoMonto($cf_totcau),0,0,'J');
	$this->Cell(85,3,'TOTAL AJUSTES CAUSADOS:    '.H::FormatoMonto($cf_ajucau[0][0]),0,0,'J');
	$this->Cell(85,3,'TOTAL CAUSA AJUSTADOS:    '.H::FormatoMonto($cf_cauaju),0,0,'J');
	$this->Ln(6);
	$this->SetX(10);
	$this->Cell(85,3,'TOTAL PAGADOS:    '.H::FormatoMonto($cf_pagado[0][0]),0,0,'J');
	$this->Cell(85,3,'TOTAL AJUSTES PAGADOS:    '.H::FormatoMonto($cf_ajupag[0][0]),0,0,'J');
	$this->Cell(85,3,'TOTAL PAGOS AJUSTADOS:    '.H::FormatoMonto($cf_pagaju),0,0,'J');
	$this->Ln(6);
	$this->SetX(10);
	$this->Cell(85,3,'TOTAL TRANSFERENCIAS:    '.H::FormatoMonto($cf_trades[0][0]),0,0,'J');
	$this->Cell(85,3,'TOTAL ADICIONES:    '.H::FormatoMonto($cf_adidis),0,0,'J');
	$this->Cell(85,3,'TOTAL ASIGNACION REAL:    '.H::FormatoMonto($cf_asireal),0,0,'J');
   }
 }
?>