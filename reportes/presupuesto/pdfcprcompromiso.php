...
class pdfreporte extends fpdf {
    ...
    function Cuerpo() {
        ...
        $primera = true;
        $totcom = 0;
        $ref2 = $this->tb[0]["refcom"];
        $this->entro = 0;
        foreach ($this->arrp as $arrp) {
            if ($ref != $arrp["refcom"]) {
                $ref = $arrp["refcom"];
                $tbcon = $this->bd->sql_contabb($ref);
                $tbcon->MoveFirst();
                $lineas = 1;
                $ycom = 170;
                if (!$primera) {
                    ...
                    if (!$tbcon->EOF) {
                        while (!$tbcon->EOF) {
                            $this->Cell(25, 5, $tbcon->fields["codcta"]);
                            $this->Cell(121, 5, $tbcon->fields["descta"]);
                            $this->Cell(50, 5, H::FormatoMonto($tbcon->fields["totcta"]));
                            $tbcon->MoveNext();
                            $ycom = $ycom + 5;
                            $lineas = $lineas + 1;
                            if (!$tbcon->EOF) {
                ...
                $this->Ln(18);
                $this->Cell(10);
                $this->Cell(88, 4, "No. COMPROMISO: " . $arrp["refcom"], 0, 0, 'L');
            ...
            if ($this->gety() > 150) {
                ...
                //buscamos el codigo contable
                $this->Ln();
                /* $this->arrp2 = $this->bd->sql_contabb($arrp["refcom"]);
                  $this->setWidths(array(25, 121, 50));
                  $this->setaligns(array('L', 'L', 'R'));
                  $this->SetY(170);
                  $lineas = 1;
