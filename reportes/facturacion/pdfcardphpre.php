<?
	...
	class pdfreporte extends fpdf
	{
		...
		function pdfreporte()
		{
		    ...
		    $this->arrp=$this->sqlp;
		    $this->sqll=$this->cardphpre->sqll($this->sqlp[0]["codalm"]);
		    $this->arrl=$this->sqll;
		    $this->otros();
		}
		...
		function Header()
		{
			...
			//	$this->Image("../../img/logo1.jpg",13,12,20);
				$this->cab->poner_cabecera_g($this,H::GetPost("titulo"),"p","s","n");
			...
			//	$this->cell(200,10,'FECHA                                      		   ',0,0,'R');
			   ...
			   // $this->cell(50,10,'DESCRIPCION DESPACHO: ');
				...
				$this->line(15,45,15,70);
				$this->line(195,45,195,70);
				$this->line(145,22,145,35);
				$this->line(145,22,195,22);
				$this->line(145,28.5,195,28.5);
				$this->line(195,22,195,35);
		...
		//		$this->cell(20,5,'Num Requisicion:  ');
			...
			//	$this->line(155,$this->getY(),155,230);
				...
				$this->SetX(15);
                         ...
                         $this->setFont("Arial","B",8);
			 $this->setwidths(array(32,77,20,15,13,20));//2+2+1+
			 $this->setaligns(array("L","L","R","R","R","R","R"));
			 //$this->setBorder(1);
			 $this->rowm(array('Cod.Material','Descripcion Material','Cant.Entreg','Pre. Unit.','IVA','Precio Total'));
			 //$this->setBorder(0);
...
//				$this->cell(30,5,'  Precio Total');
		...
		function Cuerpo()
		{
			...
			$x=0;
                        ...
                        $this->ln(18);
                        $this->SetXY(15,45);
                        $this->cell(20,5,'Nro. Pedido: '.$this->sqlp[0]['reqart']);
                        $this->SetXY(15,49);
                        $this->cell(60,5,'Localidad: '.$this->sqlp[0]["codalm"]." - ".$this->sqll[0]["nomalm"]);
                        $this->SetXY(15,53);
                        $this->cell(30,5,'Cliente: '.$this->sqlp[0]["rifpro"]);
                        $this->setFont("Arial","B",6);
                        $this->multicell(180,5,$this->sqlp[0]["nompro"]);
                        $this->SetXY(15,58);
                         $this->setFont("Arial","B",8);
                        $this->multicell(180,3,'Dirección: '.$this->sqlp[0]["dirpro"]);
                        $this->SetXY(15,63);
                        $this->cell(25,5,'Descripción: '.$this->sqlp[0]["desdph"]);
                        $this->setXY(150,24);
                        $this->cell(30,5,"N° Despacho: ".$this->sqlp[0]["dphact"]);
                        $this->setXY(150,30);
                        $this->cell(30,5,"Fecha: ".$this->sqlp[0]['fecdph']);
                        $this->setXY(55,48.5);
                        $this->setTextcolor(0,0,0);
			...
			foreach($this->sqlp as $this->sqlp)
			{
			  if($this->sqlp["dphact"]!=$ref)
			  {
				...
				$ok=true;
                        ...
                        $this->sqll=$this->cardphpre->sqll($this->sqlp["codalm"]);
                        $this->arrl=$this->sqll;
			$this->setTextcolor(0,0,150);
                        $this->setFont("Arial","B",8);
                        $this->ln(18);
                        $this->SetXY(15,45);
                        $this->cell(20,5,'Nro. Pedido: '.$this->sqlp['reqart']);
                        $this->SetXY(15,49);
                        $this->cell(60,5,'Localidad: '.$this->sqlp["codalm"]." - ".$this->sqll[0]["nomalm"]);
                        $this->SetXY(15,53);
                        $this->cell(30,5,'Cliente: '.$this->sqlp["rifpro"]);
                        $this->cell(80,5,$this->sqlp["nompro"]);
                        $this->SetXY(15,58);
                        $this->multicell(180,3,'Dirección: '.$this->sqlp["dirpro"]);
                        $this->SetXY(15,63);
                        $this->cell(25,5,'Descripción: '.$this->sqlp["desdph"]);
                        $this->setXY(150,24);
                        $this->cell(30,5,"N° Despacho: ".$this->sqlp["dphact"]);
                        $this->setXY(150,30);
                        $this->cell(30,5,"Fecha: ".$this->sqlp['fecdph']);
                        $this->setXY(55,48.5);
                        $this->setTextcolor(0,0,0);
			 ...
			 $this->setX(15);
			 $this->setwidths(array(32,78,19,14,14,20));//2+2+1
			 $this->setaligns(array("L","L","R","R","R","R"));
			 $this->rowm(array($this->sqlp["codart"],$this->sqlp["desart"],number_format($this->sqlp["candph"],2,'.',','),number_format($this->sqlp["preart"],2,'.',','),number_format($this->sqlp["monrgo"],2,'.',','),number_format($this->sqlp["pretot"],2,'.',',')));
...
/*
		  ...
		  $this->setXY(15,268);
	  	  ...
	  	  $this->cell(200,0,$this->direccion.'  -  '.$this->telefono,0,0,'C');
                  $this->setFont("Arial","B",8);
                  //$this->cell(200,0,$this->$this->cell(200,0,$this->direccion.'  -  '.$this->telefono,0,0,'C');direccion.'  -  '.$this->telefono,0,0,'C');
//$this->setFont("Arial","B",6);
//$this->cell(200,0,"Esquina Telares,Edificio Pdval Piso 1, Oficina Mezzanina, Sector San José, Distrito Capital, Municipio Libertador, Parroquia San José, Ciudad Caracas, Teléfono 0212-5557128/0212-5557138, Fax 02125557078, Zona Postal 1010.Rif. G200100249 www.pdval.gob.ve");
