	...
	class pdfreporte extends fpdf
	{
		...
		function Header()
		{
			...
			$this->fecha();
                        ...
                        $this->cell(15,10,"Hasta: ".$this->fecperhas);
                        $this->ln(10);                                               
					...
					$this->Buscar_Saldo_Anterior();
					 ...
					 if (ltrim(rtrim($arrp["titulo"]))=='TOTAL')
					 {
						...
						$this->Multicell($this->anchos[4],4,strtoupper($arrp["titulo"]." ".$arrp["nombre"]));
                                                $this->Cell($this->anchos[4],4,strtoupper($arrp["titulo"]." ".substr($arrp["nombre"],0,30)));
						$y1=$this->GetY();
						$this->SetXY($x,$y);
						...
						$this->cell(30,4,H::FormatoMonto(abs($arrp["saldo"])),0,0,'R');
                                                $this->SetXY($x,$y);   */
                                                $this->setFont("Arial","B",8);
                                                $this->setwidths(array(40,75,28,28,28,28,28));
                          			$this->setaligns(array("L","L","R","R","R","R","R"));
                        			$this->rowm(array(" ",
                                                        $arrp["titulo"]." ".$arrp["nombre"],
                                                        H::FormatoMonto(abs($this->salant)),
							H::FormatoMonto(abs($arrp["debito"])),
							H::FormatoMonto(abs($arrp["credito"])),
							H::FormatoMonto(abs($arrp["salper"])),
							H::FormatoMonto(abs($arrp["saldo"]))));
                                                $this->setFont("Arial","",8);
					 }
					 else
					 {  $sw=0;
/*
					      ...
					      //$this->MultiCell($this->anchos[4],4,strtoupper($arrp["nombre"]));
                                              $this->Cell($this->anchos[4],4,strtoupper(substr($arrp["nombre"],0,40)));
					      $y1=$this->GetY();
*/                                            
                                              if ($arrp["cargable"]=='C')
                                              {
                                              ...
                                              $this->setwidths(array(40,75,28,28,28,28,28));
                          		      $this->setaligns(array("L","L","R","R","R","R","R"));
                        		      $this->rowm(array($this->cuenta,
                                                        strtoupper($arrp["nombre"]),
                                                        H::FormatoMonto(abs($this->salant)),
							H::FormatoMonto(abs($arrp["debito"])),
							H::FormatoMonto(abs($arrp["credito"])),
							H::FormatoMonto(abs($arrp["salper"])),
