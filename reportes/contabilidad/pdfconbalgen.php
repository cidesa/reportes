    ...
    function pdfreporte() {
        ...
        $this->sql = "SELECT  A.CODCTA,A.ORDEN,
						...
						WHERE
								...
								A.CUENTA LIKE (RTRIM(B.CODCTD)||'%') ) and
...
//Return
    ...
    function Datos($i) {
        ...
        $sw = 1;
        /*         * *******DISCO NOMINA************ */
        $nombre_archivo = "/tmp/balance_general_" . strftime('%d_%m_%Y', time()) . ".xls";
            ...
            if ($tb->fields["titulo"] != " ") {
                ...
                if (($niv == ($this->nrorup) and (trim($tb->fields["cargable"] == 'C')))) {
                    ...
                    if ((($niv == (($this->nrorup) - 1) and (($this->nrorup) - 1) > 0) or
                            ...
                            ($niv == (($this->nrorup) - 6) and (($this->nrorup) - 6) > 0))) {
                ...
                if (!empty($tb_temp->fields["mov"])) {
                    $mov = $tb_temp->fields["mov"];
                }
                ...
                if ((($niv == (($this->nrorup) - 1) and (($this->nrorup) - 1) > 0) or
                        ...
                        (trim($tb->fields["cargable"] == 'C'))) {  //print $tb->fields["nombre"]."<br>";
...
//						   $this->Line($this->GetX()+3,$this->GetY()+2,$this->GetX()-30,$this->GetY()+2);
            ...
            $cadena = $codcta . "\t" . $descta . "\t" . H::FormatoMonto($tb->fields["saldo"]);
            fputs($depositos, $cadena . chr(13) . chr(10));
        ...
        $this->PutLink($dir . 'descargar.php?archivo=' . $nombre_archivo, "XLS Balance General");
        fclose($depositos);
        ##############FIN DISCO####################
    }
    ...
    function CalcularResultado() {
        ...
        if (!$tb990->EOF) {
            ...
            if ((rtrim($cuenta_resultado) == rtrim($cuenta_pasivo))) {
                ...
                $tb99 = $this->bd->select("SELECT A.SALACT as resultado
														   FROM
															 CONTABB1 A,
															 CONTABA B
														   WHERE
            ...
            if (($ingreso) > ($egreso)) {
                ...
                $this->Total_Resultado = (($pasivo + $capital + $resultado - (abs($ingreso) - abs($egreso))));
            }
        }
    }
