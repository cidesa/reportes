...
class Carordser extends baseClases
{
  ...
  $sql="SELECT DISTINCT A.ORDCOM,
	  ...
	  C.CODPAR as CODPAR1,
      F.CANORD AS CANTOT,
      (CASE WHEN A.VALMON=0.0 THEN F.COSTO ELSE F.COSTO/A.VALMON END)  AS PREART,
      (CASE WHEN A.VALMON=0.0 THEN F.TOTART ELSE F.TOTART/A.VALMON END) AS TOTART,
      RTRIM(F.DESRES) AS DESRES,
        A.DOCCOM AS DOCCOM,
      A.TIPMON,
      Z.NOMMON,
        E.TELPRO AS TELPRO, b.dtoart as dcto,a.notord as obs
      FROM CAORDCOM A LEFT OUTER JOIN TSDESMON Z ON A.TIPMON=Z.CODMON, CAARTORD B, CAREGART C,CAPROVEE E, CARESORDCOM F
      WHERE
...
//print '<pre>';print $sql;exit;
  ...
  A.coduni as coduni,
  (CASE WHEN A.VALMON=0.0 THEN monimp ELSE monimp/a.valmon END) as monto
