...
class His_np_lph_disco extends baseClases
{
    ...
    {
        $sql = "SELECT coalesce(SUM(MONTO),0) as ELMONTO FROM NPHISCON A, NPHOJINT B, NPCONTIPAPORET C  WHERE C.CODTIPAPO='0003' AND  A.CODNOM=C.CODNOM AND A.CODCON=C.CODCON AND C.TIPO='A' AND  B.CODEMP=A.CODEMP AND  B.CODEMP='$tbcodemp' AND A.FECNOM >= to_date('$fechades','dd/mm/yyyy') AND A.FECNOM <= to_date('$fechahas','dd/mm/yyyy')";
    //print"<pre> $sql=".$sql;
    return $this->select($sql);
    }
}
