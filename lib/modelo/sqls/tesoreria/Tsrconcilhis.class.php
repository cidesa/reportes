...
class Tsrconcilhis extends baseClases {
...
/*  function sqlano()
  ...
  function sqlp($ctades,$ctahas,$fecha)
  {
  	$sql="select z.*,coalesce(x.nomben,z.desmov) as desmov from (
			...
			A.RESULT,
			C.DEBCRE
			FROM tsconcilhis A  left outer join  TSDEFBAN B on (a.numcue=b.numcue) ,TSTIPMOV C
			WHERE
			CASE WHEN A.MONLIB=0 THEN A.MOVBAN ELSE A.MOVLIB END =C.CODTIP AND
			A.NUMCUE >= RTRIM('$ctades') AND
			A.NUMCUE <= RTRIM('$ctahas') AND
			a.mescon = 	to_char(to_date('$fecha','dd/mm/yyyy'),'mm')	and
			a.anocon = 	to_char(to_date('$fecha','dd/mm/yyyy'),'yyyy')	and
...
*/
  function sqlresumen($ctades,$ctahas,$fecha)
  {
  	$sql="SELECT
			...
			and a.mescon = 	to_char(to_date('$fecha','dd/mm/yyyy'),'mm')
			and a.anocon = 	to_char(to_date('$fecha','dd/mm/yyyy'),'yyyy')
