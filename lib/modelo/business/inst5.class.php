...
class inst5 extends baseClases
{
  ...
  public static function catalogo_codprePrerasiini_accesp($objhtml)
  {
   ...
   from cpdeftit a where ( substr(codpre,1,5) like '%V_0%' ) and length(trim(codpre))<=5 order by codpre";
	...
	{
		$sql="select cuenta as codigo, desccta as descripcion from forcfgrepins where nrofor='0406' and (cuenta like '%V_0%' AND desccta like '%V_1%') order by cuenta";

		$catalogo = array(
		    $sql,
		    array('Codigo de la Cuenta','Descripcion de la Cuenta'),
		    array($objhtml),
		    array('codigo'),
		    100
		    );
