...
class Contabilidad extends baseClases
{
	...
	{
		$sql="select distinct a.numcom as numcom, c.descom, b.descta, a.codcta as codcta from contabc1 a, contabb b, contabc c where a.numcom=c.numcom and a.codcta=b.codcta and ( a.numcom like '%V_0%' and c.descom like '%V_1%') order by a.numcom";
	  ...
	  public static function SQLContaba_nivel2($valor)
	  {
		...
		public function ReactualizarSaldosAnteriores()
		{
			$temp=$this->select("SELECT
			   ...
			   $temp=$this->select("SELECT SUBSTR(CODCTA,1,18) as CUENTA,SUM(COALESCE(SALANT,0)) as SALDO FROM CONTABB WHERE upper(CARGAB)='C' GROUP BY SUBSTR(CODCTA,1,18)");
					foreach($temp as $arr){
				...
				//--------------
                          ...
                          $temp=$this->select("SELECT SUBSTR(CODCTA,1,14) as CUENTA,SUM(COALESCE(SALANT,0)) as SALDO FROM CONTABB WHERE upper(CARGAB)='C' GROUP BY SUBSTR(CODCTA,1,14)");
                                        foreach($temp as $arr){
                                                $this->bd->actualizar("update contabb set salant=coalesce('".$arr["saldo"]."',0) where codcta=rpad(coalesce('".$arr["cuenta"]."',32,' '))");
					 }
				//--------------

			   $temp=$this->select("SELECT SUBSTR(CODCTA,1,11) as CUENTA,SUM(COALESCE(SALANT,0)) as SALDO FROM CONTABB WHERE upper(CARGAB)='C' GROUP BY SUBSTR(CODCTA,1,11)");
					foreach($temp as $arr){
						$this->bd->actualizar("update contabb set salant=coalesce('".$arr["saldo"]."',0) where codcta=rpad(coalesce('".$arr["cuenta"]."',32,' '))");
					 }
				//--------------

			   $temp=$this->select("SELECT SUBSTR(CODCTA,1,08) as CUENTA,SUM(COALESCE(SALANT,0)) as SALDO FROM CONTABB WHERE upper(CARGAB)='C' GROUP BY SUBSTR(CODCTA,1,08)");
					foreach($temp as $arr){
						$this->bd->actualizar("update contabb set salant=coalesce('".$arr["saldo"]."',0) where codcta=rpad(coalesce('".$arr["cuenta"]."',32,' '))");
					}
				//--------------

			   $temp=$this->select("SELECT SUBSTR(CODCTA,1,05) as CUENTA,SUM(COALESCE(SALANT,0)) as SALDO FROM CONTABB WHERE upper(CARGAB)='C' GROUP BY SUBSTR(CODCTA,1,05)");
					foreach($temp as $arr){
						$this->bd->actualizar("update contabb set salant=coalesce('".$arr["saldo"]."',0) where codcta=rpad(coalesce('".$arr["cuenta"]."',32,' '))");
					}
				//--------------

			   $temp=$this->select("SELECT SUBSTR(CODCTA,1,03) as CUENTA,SUM(COALESCE(SALANT,0)) as SALDO FROM CONTABB WHERE upper(CARGAB)='C' GROUP BY SUBSTR(CODCTA,1,03)");
					foreach($temp as $arr){
						$this->bd->actualizar("update contabb set salant=coalesce('".$arr["saldo"]."',0) where codcta=rpad(coalesce('".$arr["cuenta"]."',32,' '))");
					}
				//--------------
		...
		function ActualizarMaestro(){
			 $temp=$this->select("select substr(codcta,1,18) as codcta,
			...
			foreach($temp as $arr){
				...
				//--------------
			$temp=$this->select("select substr(codcta,1,18) as codcta, RTRIM(to_char(a.feccom,'MM')) as mes,sum(case when a.debcre='D' then a.monasi else 0 end) AS DEB,sum(case when a.debcre='C' then a.monasi else 0 end) as CRE from contabc1 a, contabc b where a.feccom = b.feccom and b.stacom='A' group by substr(codcta,1,18),RTRIM(to_char(a.feccom,'MM'))");
			foreach($temp as $arr){
				$this->bd->actualizar("update contabb1 set totdeb=COALESCE('".$arr["deb"]."',0),totcre=COALESCE('".$arr["cre"]."',0) where codcta=rpad('".$arr["codcta"]."',32,' ') and pereje='".$arr["mes"]."'");
                                      
                        }
                        //--------------
                        $temp=$this->select("select substr(codcta,1,14) as codcta, RTRIM(to_char(a.feccom,'MM')) as mes,sum(case when a.debcre='D' then a.monasi else 0 end) AS DEB,sum(case when a.debcre='C' then a.monasi else 0 end) as CRE from contabc1 a, contabc b where a.feccom = b.feccom and b.stacom='A' group by substr(codcta,1,14),RTRIM(to_char(a.feccom,'MM'))");
                        foreach($temp as $arr){
                                $this->bd->actualizar("update contabb1 set totdeb=COALESCE('".$arr["deb"]."',0),totcre=COALESCE('".$arr["cre"]."',0) where codcta=rpad('".$arr["codcta"]."',32,' ') and pereje='".$arr["mes"]."'");
			}

				//--------------
			$temp=$this->select("select substr(codcta,1,11) as codcta, RTRIM(to_char(a.feccom,'MM')) as mes,sum(case when a.debcre='D' then a.monasi else 0 end) AS DEB,sum(case when a.debcre='C' then a.monasi else 0 end) as CRE from contabc1 a, contabc b where a.feccom = b.feccom and b.stacom='A' group by substr(codcta,1,11),RTRIM(to_char(a.feccom,'MM'))");
			foreach($temp as $arr){
				$this->bd->actualizar("update contabb1 set totdeb=COALESCE('".$arr["deb"]."',0),totcre=COALESCE('".$arr["cre"]."',0) where codcta=rpad('".$arr["codcta"]."',32,' ') and pereje='".$arr["mes"]."'");
			}

				//--------------
			$temp=$this->select("select substr(codcta,1,8) as codcta, RTRIM(to_char(a.feccom,'MM')) as mes,sum(case when a.debcre='D' then a.monasi else 0 end) AS DEB,sum(case when a.debcre='C' then a.monasi else 0 end) as CRE from contabc1 a, contabc b where a.feccom = b.feccom and b.stacom='A' group by substr(codcta,1,8),RTRIM(to_char(a.feccom,'MM'))");
			foreach($temp as $arr){
				$this->bd->actualizar("update contabb1 set totdeb=COALESCE('".$arr["deb"]."',0),totcre=COALESCE('".$$arr["cre"]."',0) where codcta=rpad('".$arr["codcta"]."',32,' ') and pereje='".$arr["mes"]."'");
			}

				//--------------
			$temp=$this->select("select substr(codcta,1,5) as codcta, RTRIM(to_char(a.feccom,'MM')) as mes,sum(case when a.debcre='D' then a.monasi else 0 end) AS DEB,sum(case when a.debcre='C' then a.monasi else 0 end) as CRE from contabc1 a, contabc b where a.feccom = b.feccom and b.stacom='A' group by substr(codcta,1,5),RTRIM(to_char(a.feccom,'MM'))");
			foreach($temp as $arr){
				$this->bd->actualizar("update contabb1 set totdeb=COALESCE('".$arr["deb"]."',0),totcre=COALESCE('".$arr["cre"]."',0) where codcta=rpad('".$arr["codcta"]."',32,' ') and pereje='".$arr["mes"]."'");
			}

				//--------------
			$temp=$this->select("select substr(codcta,1,3) as codcta, RTRIM(to_char(a.feccom,'MM')) as mes,sum(case when a.debcre='D' then a.monasi else 0 end) AS DEB,sum(case when a.debcre='C' then a.monasi else 0 end) as CRE from contabc1 a, contabc b where a.feccom = b.feccom and b.stacom='A' group by substr(codcta,1,3),RTRIM(to_char(a.feccom,'MM'))");
			foreach($temp as $arr){
				$this->bd->actualizar("update contabb1 set totdeb=COALESCE('".$arr["deb"]."',0),totcre=COALESCE('".$arr["cre"]."',0) where codcta=rpad('".$arr["codcta"]."',32,' ') and pereje='".$arr["mes"]."'");
			}
